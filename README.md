...
Trained a LightGBM `LGBMRanker` (LambdaRank objective) on **905
(source, candidate) pairs** labelled by **Claude Haiku 4.5** against a
product-grounded rubric (`hangpost_matching.llm_judge.JUDGE_SYSTEM_PROMPT`).
Evaluated on a held-out test split at strict relevance (rating = 4 only,
the rubric's "strong match" tier):

| Ranker                       | NDCG@10 | MAP@10 | P@10  | Lift over random (ΔNDCG) |
|------------------------------|---------|--------|-------|--------------------------|
| `random` (baseline)          | 0.280   | 0.140  | 0.278 | —                        |
| `rules_only` (Phase 1)       | 0.877   | 0.793  | 0.656 | +0.597                   |
| `rules+embeddings` (Phase 2) | 0.886   | 0.806  | 0.667 | +0.606                   |
| **`learned`** (Phase 3)      | **0.967** | **0.936** | **0.733** | **+0.687**           |

**The learned LightGBM ranker beats the hand-tuned rules baseline by
+0.090 NDCG@10** — that's the Phase 3 teacher → student distillation
working as advertised. Phase 2 sentence-transformer embeddings add a
small but consistent +0.009 NDCG over Phase 1 alone.

> **Methodology caveats** (read these before quoting the numbers):
> - **Test set is 9 queries** (30 source queries × 30% holdout). Small —
>   confidence intervals are wide. The +0.09 NDCG gap is large enough to
>   survive that, but firm conclusions need a larger labelling pass
>   (`--queries 60+`).
> - **Judged candidate pool is biased toward relevant.** For each source,
>   `scripts/label.py` judges the top-15 from the rules ranker + 15
>   uniformly-random candidates — not all 999. The top-15 are by
>   construction the rules ranker's best guesses, so absolute precision
>   is inflated. The *gap* between rankers is the meaningful number.
> - **Labels are LLM-judged, not from real user behaviour.** Substitute
>   real accept/reject/retention outcomes as soon as production data
>   exists; treat the judge labels as a stopgap and a teacher signal,
>   not ground truth.
>
> Run `WITH_GOLD=1 ./scripts/refresh_results.sh` (~$3-5 extra in
> Anthropic tokens) to additionally re-judge a stratified subset with
> Sonnet 4.6 and print quadratic-weighted Cohen's κ for inter-rater
> agreement — that's how you quantify how lenient Haiku is on this
> rubric.
...
python scripts/label.py --model claude-haiku-4-5-20251001 --no-thinking \
    --queries 30 --top-k 15 --random-k 15
